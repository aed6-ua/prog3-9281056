...
public class FightersPreTest {
	...
	public void testCopy() {
		...
		Fighter ft = FighterFactory.createFighter("TIEBomber", rebelShip);
		assertEquals("copy TIEBomber",ft,ft.copy());
		AWing faa = (AWing)fa;
		AWing facopy = (AWing) fa.copy();
		assertEquals("copy AWing as AWing",faa,facopy);

		Fighter ftf = FighterFactory.createFighter("TIEFighter", rebelShip);
		Fighter fti = FighterFactory.createFighter("TIEInterceptor", rebelShip);
		assertEquals("copy TIEInterceptor",fti,fti.copy());
		TIEFighter ftff = (TIEFighter)ftf;
		TIEFighter ftfcopy = (TIEFighter) ftf.copy();
		assertEquals("copy TIEFighter as TIEFighter",ftff,ftfcopy);
	}
